...
export class BenchmarkingService {
  ...
  async getCurrentProjectId(): Promise<number> {
    ...
    try {
      ...
      this.logger.error(
        `Esquema de Benchmarking no listo (¿faltan proyectos?): ${error.message}`,
      );
      throw error;
    }
  ...
  async runRegisteredQueries(): Promise<void> {
    ...
    for (const q of queries) {
      try {
        ...
        this.logger.error(
          `Error al ejecutar consulta "${q.query_description}": ${error.message}`,
        );
      }
    }
  }
  ...
  async processDailySnapshot(authHeader: string): Promise<any> {
    ...
    if (metrics.length === 0) {
      throw new BadRequestException(
        "No hay métricas acumuladas (calls > 0) para exportar.",
      );
    }
  ...
  async processHistoricalSnapshot(authHeader: string, days = 30): Promise<any> {
    ...
    if (metrics.length === 0) {
      throw new BadRequestException(
        "No hay métricas base para generar historial. Ejecuta algunas consultas primero.",
      );
    }
  ...
  private async sendToBigQuery(accessToken: string, rows: any[]): Promise<any> {
    try {
      ...
      throw new BadRequestException(
        `Fallo en envío a BigQuery: ${error.message}`,
      );
    }
  }
  ...
  async getQueryMetrics(limit = 20): Promise<any[]> {
    try {
      ...
      this.logger.warn(`pg_stat_statements no disponible o desactivado en la base de datos: ${error.message}`);
      return [];
    }
  }

  async verifyStatus(accessToken: string): Promise<{
    project_id: number;
    token_valid: boolean;
    ...
    const tokenValid = await this.oauthService.validateToken(accessToken);

    const metrics = await this.getQueryMetrics(1);

    return {
      project_id: projectId,
      token_valid: tokenValid,
      metrics_ready: metrics.length > 0,
