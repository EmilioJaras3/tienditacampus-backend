...
export class BenchmarkingController {
  ...
  async takeSnapshot(@Headers("x-google-token") googleToken: string) {
    if (!googleToken) {
      throw new UnauthorizedException(
        "Se requiere x-google-token (Google OAuth)",
      );
    }
  ...
  async takeHistoricalSnapshot(
    ...
    return this.benchmarkingService.processHistoricalSnapshot(
      googleToken,
      days || 30,
    );
  }
  ...
  @Post("verify-status")
  async verifyStatus(@Headers("x-google-token") googleToken: string) {
    if (!googleToken) {
      throw new UnauthorizedException(
        "Se requiere x-google-token (Google OAuth)",
      );
    ...
    return this.benchmarkingService.verifyStatus(googleToken);
  }
