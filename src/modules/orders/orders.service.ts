...
export class OrdersService {
  ...
  async acceptOrder(orderId: string, seller: User): Promise<Order> {
    ...
    if (order.sellerId !== seller.id)
      throw new BadRequestException(
        "No tienes permiso para aceptar esta orden",
      );
    if (order.status !== "requested")
      throw new BadRequestException("La orden no está en estado solicitado");
  ...
  async rejectOrder(orderId: string, seller: User): Promise<Order> {
    ...
    if (order.sellerId !== seller.id)
      throw new BadRequestException(
        "No tienes permiso para rechazar esta orden",
      );
    if (order.status !== "requested")
      throw new BadRequestException("Solo puedes rechazar órdenes solicitadas");
  ...
  async deliverOrder(orderId: string, user: User): Promise<Order> {
    ...
    try {
      ...
      for (const orderItem of order.items) {
        let saleDetail = dailySale.details.find(
          (d) => d.productId === orderItem.productId,
        );
        ...
        if (!saleDetail) {
          ...
          saleDetail.unitCost = await this.getUnitCost(
            queryRunner,
            orderItem.productId,
          );
      ...
      for (const d of dailySale.details) {
        ...
        const investmentContrib =
          d.quantityPrepared > 0 ? d.quantityPrepared : d.quantitySold;
      ...
      if (unitsSold > 0) {
        ...
        const avgUnitCost =
          unitsPrepared > 0 ? totalInvestment / unitsPrepared : 0;
