...
export class DeliverOrderUseCase {
    ...
    return this.transactionManager.execute(async (queryRunner) => {
      ...
      if (order.status !== "pending" && order.status !== "accepted") {
        throw new BadRequestException(
          "Esta orden ya fue procesada o cancelada",
        );
      }
      ...
      await this.auditService.log({
        ...
        entityType: "orders",
        entityId: order.id,
        description: `Pedido #${order.id.substring(0, 8)} marcado como entregado/completado por ${user.email}`,
        metadata: { totalAmount: order.totalAmount, buyerId: order.buyerId },
      });
  ...
  private recalculateAggregates(dailySale: DailySale) {
    ...
    if (unitsSold > 0) {
      ...
      if (unitMargin > 0) {
        dailySale.breakEvenUnits = Number(
          (totalInvestment / unitMargin).toFixed(2),
        );
      }
    }
  }
