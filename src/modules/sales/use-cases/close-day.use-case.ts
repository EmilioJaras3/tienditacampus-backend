...
export class CloseDayUseCase {
  ...
  async execute(
    ...
    if (dailySale.isClosed)
      throw new BadRequestException("El día ya está cerrado");
      ...
      if (detail) {
        ...
        if (waste > detail.quantityPrepared) {
          throw new BadRequestException(
            `Merma (${waste}) no puede exceder preparado (${detail.quantityPrepared}) para ${detail.product.name}`,
          );
        }
        ...
        if (unitsSold > 0) {
          await this.salesRepository.consumeInventory(
            item.productId,
            user.id,
            unitsSold,
          );
