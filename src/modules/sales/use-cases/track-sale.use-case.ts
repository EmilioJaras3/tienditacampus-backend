...
export class TrackSaleUseCase {
  ...
  async execute(trackDto: TrackSaleDto, user: User): Promise<any> {
    ...
    if (!dailySale) {
      throw new NotFoundException(
        "No active daily sale found for today. Please initialize the day first.",
      );
    }

    const detail = dailySale.details.find(
      (d) => d.productId === trackDto.productId,
    );
    if (!detail) {
    ...
    if (
      ...
      trackDto.quantitySold + trackDto.quantityLost
    ) {
      throw new BadRequestException(
        "Cannot sell/lose more than prepared quantity",
      );
    }
  ...
  private async recalculateHeader(dailySaleId: string): Promise<any> {
    ...
    if (sale.unitsSold > 0) {
      ...
      const avgUnitCost =
        unitsPrepared > 0 ? Number(sale.totalInvestment) / unitsPrepared : 0;
      ...
      if (unitMargin > 0) {
        sale.breakEvenUnits = Number(
          (Number(sale.totalInvestment) / unitMargin).toFixed(2),
        );
      } else {
