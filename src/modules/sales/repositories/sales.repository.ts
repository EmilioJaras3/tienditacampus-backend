...
export interface ISalesRepository {
  ...
  getHistory(
    ...
    page?: number,
    limit?: number,
  ): Promise<{ data: DailySale[]; total: number; page: number; limit: number }>;
...
export class SalesRepository
  extends BaseRepository<DailySale>
  implements ISalesRepository
{
  ...
  async getROI(sellerId: string): Promise<{
    investment: number;
    revenue: number;
    netProfit: number;
    roi: number;
  ...
  async getPrediction(sellerId: string): Promise<any> {
    ...
    const suggestions = Object.keys(productSales)
      .map((productId) => {
  ...
  async consumeInventory(
    ...
    console.log(
      `Consuming ${units} units of product ${productId} for user ${userId}`,
    );
  }
  ...
  async closeDay(dailySale: DailySale): Promise<DailySale> {
    ...
    if (dailySale.unitsSold > 0) {
      ...
      const avgUnitCost =
        unitsPrepared > 0
          ? Number(dailySale.totalInvestment) / unitsPrepared
          : 0;

      const wasteRate =
        unitsPrepared > 0 ? dailySale.unitsLost / unitsPrepared : 0;
      ...
      if (unitMargin > 0) {
        dailySale.breakEvenUnits = Number(
          (Number(dailySale.totalInvestment) / unitMargin).toFixed(2),
        );
      } else {
